document.addEventListener('DOMContentLoaded', () => {
    ...
    const NOTE_FREQUQUENCIES = { 'G3': 196.00, 'C4': 261.63, 'Dsharp4': 311.13, 'G4': 392.00, 'Gsharp4': 415.30, 'Asharp4': 466.16 };
    ...
    function playNote(note, startTime, duration) { if (!audioCtx) return; const f = NOTE_FREQUQUENCIES[note]; if (!f) return; const o = audioCtx.createOscillator(), n = audioCtx.createGain(); o.connect(n); n.connect(masterGain); o.type = 'square'; o.frequency.setValueAtTime(f, startTime); n.gain.setValueAtTime(0, startTime); n.gain.linearRampToValueAtTime(0.5, startTime + 0.01); n.gain.linearRampToValueAtTime(0, startTime + duration); o.start(startTime); o.stop(startTime + duration); }
    ...
    function updateUI() {
        ...
        const totalLaps = 50; // Using the original constant as track-specific laps were not fully implemented

        // Update New Player Header
        ...
        gameState.cars.forEach((car, index) => {
            ...
            if (index > 0) {
                ...
                gapText = `+${(car.speed > 0 ? diff / car.speed / FPS / TIME_SCALE_FACTOR: 999).toFixed(2)}s`;
            }
